...
{
	/* copy .text from FLASH to CODE_RAM */
	extern uint32_t __code_ram_load, __code_ram_start, __code_ram_end;
	uint32_t *src = &__code_ram_load;
	uint32_t *dst = &__code_ram_start;
	uint32_t *dst_end = &__code_ram_end;
	while (dst < dst_end)
		*dst++ = *src++;
